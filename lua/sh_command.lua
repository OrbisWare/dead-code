  ...
  function cu.cmd.AddConCommand(name, callback, permi, help)
  	...
  	concommand.Add(name, function(ply, cmd, args, raw)
  		...
  		if permi and !ply:HasPermission(permi) then
  			...
  			cu.log.Write("COMMAND", "%s tried to run command %s", cu.util.PlayerName(ply), name)
  			return
  		end
  		...
  		if !run then
  			...
  			cu.log.Write("COMMAND", "%s tried to run command %s. Error %s", cu.util.PlayerName(ply), name, err)
  			return
  		end
  	end, nil, help)
  end
